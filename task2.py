...
class TokenType:
    ...
    PLUS = "PLUS"
    MINUS = "MINUS"
    MUL = "MUL"      # токен множення
    DIV = "DIV"      # токен ділення
    OPENBR = "OPENBR"  # токен відкритої дужки
    CLOSEBR = "CLOSEBR"  # токен закритої дужки
    EOF = "EOF"
...
class Lexer:
    ...
    def get_next_token(self):
        ...
        while self.current_char is not None:
            ...
            if self.current_char == "(":
                self.advance()
                return Token(TokenType.OPENBR, "(")

            if self.current_char == ")":
                self.advance()
                return Token(TokenType.CLOSEBR, ")")
...
class Parser:
    ...
    def factor(self):
        ...
        if token.type == TokenType.INTEGER:
            ...
            return Num(token)
        elif token.type == TokenType.OPENBR:
            self.eat(TokenType.OPENBR)
            node = self.expr()
            self.eat(TokenType.CLOSEBR)
            return node
        else:
            self.error()
